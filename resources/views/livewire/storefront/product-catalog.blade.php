    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-8">
            ...
            <p class="mt-1 text-gray-500">
                @if($search || $categorySlug || $minPrice || $maxPrice)
                    ...
                    @if($activeCategory) — <span class="text-rose-500 font-medium">{{ $activeCategory->name }}</span>@endif
                @else
        ...
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            >
                ...
                <div x-show="open" x-transition class="lg:!block" style="display: none;" :style="window.innerWidth >= 1024 ? 'display:block' : ''">
                    <div class="bg-white rounded-2xl border border-gray-100 p-5 space-y-6 lg:sticky lg:top-24">
                        ...
                        @if($search || $categorySlug || $minPrice || $maxPrice || $sort !== 'latest')
                            <div class="flex items-center justify-between">
                                <span class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Filter Aktif</span>
                                <button
                        ...
                        <div>
                            ...
                            <div class="space-y-1">
                                ...
                                @foreach($categories as $cat)
                                    <button
                                        wire:click="$set('categorySlug', '{{ $cat->slug }}')"
                                        class="w-full text-left px-3 py-2 rounded-lg text-sm transition
                                    ...
                                    @foreach($cat->children as $child)
                                        <button
                                            wire:click="$set('categorySlug', '{{ $child->slug }}')"
                                            class="w-full text-left pl-7 pr-3 py-1.5 rounded-lg text-sm transition
                                                {{ $categorySlug === $child->slug ? 'bg-rose-50 text-rose-600 font-medium' : 'text-gray-500 hover:bg-gray-50' }}"
                                        >
                                            <span class="text-gray-300 mr-1">└</span> {{ $child->name }}
                                        </button>
                                    @endforeach
                                @endforeach
                            </div>
                        ...
                        <div>
                            ...
                            <div class="space-y-2">
                                ...
                                @if($minPrice || $maxPrice)
                                    <button
                                        wire:click="$set('minPrice', ''); $set('maxPrice', '')"
                                        class="text-xs text-rose-500 hover:text-rose-700"
                                    >Hapus filter harga</button>
                                @endif
                            </div>
                        </div>
                                ...
                                @foreach([
                                    ...
                                    <button
                                        ...
                                        class="w-full text-left px-3 py-2 rounded-lg text-sm transition flex items-center gap-2
                                            {{ $sort === $value ? 'bg-rose-50 text-rose-600 font-medium' : 'text-gray-600 hover:bg-gray-50' }}"
                                    >
                                        @if($sort === $value)
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="flex flex-wrap items-center gap-2 mb-5">
                    ...
                    @if($categorySlug && $activeCategory)
                        <span class="inline-flex items-center gap-1 px-2.5 py-1 bg-rose-50 text-rose-600 text-xs font-medium rounded-full">
                            {{ $activeCategory->name }}
                    ...
                    @endif
                    @if($minPrice || $maxPrice)
                        <span class="inline-flex items-center gap-1 px-2.5 py-1 bg-amber-50 text-amber-600 text-xs font-medium rounded-full">
                            Rp {{ $minPrice ? number_format((float)$minPrice, 0, ',', '.') : '0' }}
                            — {{ $maxPrice ? 'Rp '.number_format((float)$maxPrice, 0, ',', '.') : '∞' }}
                            <button wire:click="$set('minPrice', ''); $set('maxPrice', '')" class="hover:text-amber-800">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                            </button>
                        </span>
                    @endif
                ...
                <div wire:loading.class="opacity-50 pointer-events-none" class="transition-opacity duration-200">
                    @if($products->isEmpty())
                        <div class="text-center py-20">
                            <svg class="mx-auto w-16 h-16 text-gray-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <button wire:click="clearFilters" class="mt-4 text-sm text-rose-500 hover:text-rose-700 font-medium">
                                Reset semua filter
                            </button>
                        </div>
                    @else
                            ...
                            @foreach($products as $product)
                                @php
                                    $images = $product->variants->isNotEmpty()
                                        ? $product->variants->map(fn($v) => $v->image ?: $product->thumbnail)->filter()->values()->toArray()
