<div x-data="{
        ...
        <div class="flex items-center justify-between h-16">
            ...
            <nav class="hidden lg:flex items-center gap-1">
                ...
                <div class="relative" @mouseenter="catOpen = true" @mouseleave="catOpen = false">
                    ...
                    <div x-data="{ expandedCategory: null }" x-show="catOpen" x-transition:enter="transition ease-out duration-150"
                        ...
                        <a href="{{ route('products') }}" wire:navigate @click="catOpen = false"
                            ...
                            @if ($cat->children->isNotEmpty())
                                <div class="space-y-1 px-2">
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('products', ['category' => $cat->slug]) }}" wire:navigate
                                            ...
                                            class="flex-1 px-4 py-2 text-sm font-medium rounded-lg transition
                                                  {{ request('category') === $cat->slug ? 'bg-rose-50 text-rose-600' : 'text-gray-700 hover:bg-rose-50 hover:text-rose-600' }}">
                                            {{ $cat->name }}
                                        </a>
                                        <button type="button"
                                    ...
                                    <div x-show="expandedCategory === {{ $cat->id }}" x-transition
                                        ...
                                        <a href="{{ route('products', ['category' => $cat->slug]) }}" wire:navigate
                                            @click="catOpen = false"
                                            class="block px-4 py-2 text-sm text-gray-500 italic rounded-lg hover:bg-rose-50 hover:text-rose-600">
                                            Semua {{ $cat->name }}
                                        </a>
                                        @foreach ($cat->children as $child)
                                            <a href="{{ route('products', ['category' => $child->slug]) }}"
                                                wire:navigate @click="catOpen = false"
                                                class="block px-4 py-2 text-sm rounded-lg transition
                                                      {{ request('category') === $child->slug ? 'text-rose-500 bg-rose-50' : 'text-gray-700 hover:bg-rose-50 hover:text-rose-600' }}">
                                                {{ $child->name }}
                                            </a>
                                        @endforeach
                                    </div>
            ...
            <div class="flex items-center gap-2">
                ...
                {{-- WA icon only on xs --}}
                <a href="https://wa.me/{{ \App\Models\Setting::get('whatsapp_number', '6281234567890') }}"
                    target="_blank" class="sm:hidden p-2 text-green-500 rounded-lg hover:bg-green-50 transition">
    ...
    <div x-show="mobileOpen" x-transition:enter="transition ease-out duration-200"
        ...
        <div class="px-4 py-3 space-y-0.5">
            ...
            {{-- Semua Produk --}}
            <a href="{{ route('products') }}" wire:navigate @click="mobileOpen = false"
                class="flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-lg transition
            ...
            {{-- Kategori accordion --}}
            <div>
                <button @click="mobileCatOpen = !mobileCatOpen"
                    ...
                    <svg class="w-4 h-4 transition-transform duration-200 shrink-0"
                        :class="mobileCatOpen ? 'rotate-180' : ''" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                    </svg>
                </button>
                ...
                {{-- Accordion content --}}
                <div x-show="mobileCatOpen" x-transition:enter="transition ease-out duration-200"
                    x-transition:enter-start="opacity-0 -translate-y-1"
                    x-transition:enter-end="opacity-100 translate-y-0"
                    x-transition:leave="transition ease-in duration-150"
                    x-transition:leave-start="opacity-100 translate-y-0"
                    x-transition:leave-end="opacity-0 -translate-y-1"
                    class="mt-1 ml-3 pl-3 border-l-2 border-rose-100 space-y-0.5" style="display:none">
                    @foreach ($categories as $cat)
                        @if ($cat->children->isNotEmpty())
                            <div class="space-y-1">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('products', ['category' => $cat->slug]) }}" wire:navigate
                                        ...
                                        class="flex-1 px-3 py-2 text-sm font-medium rounded-lg transition
                                              {{ request('category') === $cat->slug ? 'text-rose-500 bg-rose-50' : 'text-gray-700 hover:bg-rose-50 hover:text-rose-600' }}">
                                        {{ $cat->name }}
                                    </a>
                                    <button type="button"
                                        @click.stop="expandedMobileCategory = expandedMobileCategory === {{ $cat->id }} ? null : {{ $cat->id }}"
                                        class="p-2 rounded-lg text-gray-500 hover:bg-gray-100 hover:text-gray-700 transition">
                                        <svg class="w-4 h-4 transition-transform"
                                            :class="expandedMobileCategory === {{ $cat->id }} ? 'rotate-180' : ''"
                                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M19 9l-7 7-7-7" />
                                        </svg>
                                ...
                                <div x-show="expandedMobileCategory === {{ $cat->id }}" x-transition
                                    class="space-y-1 pl-5 border-l-2 border-rose-100">
                                    <a href="{{ route('products', ['category' => $cat->slug]) }}" wire:navigate
                                        @click="mobileOpen = false"
                                        class="block px-3 py-2 text-sm text-gray-500 italic rounded-lg hover:bg-rose-50 hover:text-rose-600">
                                        Semua {{ $cat->name }}
                                    </a>
                                    @foreach ($cat->children as $child)
                                        <a href="{{ route('products', ['category' => $child->slug]) }}" wire:navigate
                                            ...
                                            class="block px-3 py-2 text-sm rounded-lg transition
                                                  {{ request('category') === $child->slug ? 'text-rose-500 bg-rose-50' : 'text-gray-500 hover:bg-rose-50 hover:text-rose-600' }}">
                                            {{ $child->name }}
                                        </a>
                                    @endforeach
                                </div>
            ...
            <div class="pt-2 mt-1 border-t border-gray-100">
                <a href="https://wa.me/{{ \App\Models\Setting::get('whatsapp_number', '6281234567890') }}"
                    target="_blank" @click="mobileOpen = false"
