            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-gray-100">
                        <th class="text-left px-6 py-4 text-xs font-medium text-gray-500 uppercase tracking-wider">Nama</th>
                        <th class="text-left px-6 py-4 text-xs font-medium text-gray-500 uppercase tracking-wider">Tipe</th>
                    ...
                    @foreach($parents as $parent)
                        <tr class="hover:bg-gray-50/50 transition">
                            ...
                            <td class="px-6 py-4 text-right space-x-2">
                                <button wire:click="openEdit({{ $parent->id }})" class="text-sm text-blue-600 hover:text-blue-800">Edit</button>
                                <button wire:click="delete({{ $parent->id }})" wire:confirm="Hapus kategori '{{ $parent->name }}'?" class="text-sm text-red-500 hover:text-red-700">Hapus</button>
                            </td>
                        </tr>
                        ...
                        @foreach($children->where('parent_id', $parent->id) as $child)
                            <tr class="bg-gray-50/30 hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-3 text-right space-x-2">
                                    <button wire:click="openEdit({{ $child->id }})" class="text-sm text-blue-600 hover:text-blue-800">Edit</button>
                                    <button wire:click="delete({{ $child->id }})" wire:confirm="Hapus subkategori '{{ $child->name }}'?" class="text-sm text-red-500 hover:text-red-700">Hapus</button>
                                </td>
                            </tr>
                        @endforeach
                    @endforeach
